...
def compute_first_free_slots(arcade_game):
    ...
    for column in range(columns_num):
        ...
        current_row = 0
        column_coord = column * (tile_size + arcade_game.level_data["offset"])
        row_coord = current_row * tile_size

        # Check if the first row is immediately free
        if not arcade.get_sprites_at_point((column_coord, row_coord + 1), arcade_game.scene["Platforms"]):
            free = True
        else:
            free = False
        ...
        while not free:
            current_row += 1
            row_coord = current_row * tile_size
            if not arcade.get_sprites_at_point((column_coord, row_coord), arcade_game.scene["Platforms"]):
                free = True

        print(f"row {column}", column_coord, row_coord)
